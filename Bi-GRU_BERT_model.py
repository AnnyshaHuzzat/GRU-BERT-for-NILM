...
class biGRUBERT(nn.Module):
    def __init__(self, args):
        ...
        self.gru = nn.GRU(input_size=self.hidden, hidden_size=self.hidden, num_layers=1, batch_first=True, bidirectional=True)


        self.transformer_blocks = nn.ModuleList([TransformerBlock(
            self.hidden * 2, self.heads, self.hidden * 8, self.dropout_rate) for _ in range(self.n_layers)])

        self.deconv = nn.ConvTranspose1d(
            in_channels=self.hidden * 2, out_channels=self.hidden, kernel_size=4, stride=2, padding=1)
