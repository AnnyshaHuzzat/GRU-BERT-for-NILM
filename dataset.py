...
class AbstractDataset(metaclass=ABCMeta):
    ...
    def _get_rawdata_root_path(self):
        return Path('Enter the path of your Datasets')
...
class REDD_LF_Dataset(AbstractDataset):
    ...
    def _if_data_exists(cls):
        folder = Path("Enter the path of the REDD dataset.")
        first_file = folder.joinpath('house_1', 'redd_house_1.csv')
        return first_file.is_file()

    def load_data(self):
        # Ensure appliance names are valid
        for appliance in self.appliance_names:
            assert appliance in ['dishwasher', 'fridge', 'microwave', 'washer_dryer']
        ...
        if not self._if_data_exists():
            ...
            for house_id in self.house_indicies:
                ...
                csv_file = house_folder.joinpath(f'redd_house_{house_id}.csv')

                if not csv_file.exists():
                    print(f"Skipping house {house_id}: CSV file not found")
                    continue
                ...
                if 'Unnamed: 0' in house_data.columns:
                    house_data = house_data.drop(columns=['Unnamed: 0'])

                # Add missing appliance columns
                for appliance in self.appliance_names:
                    if appliance not in house_data.columns:
                        house_data[appliance] = 0

                # Keep only the aggregate + appliance columns
                cols = ['aggregate'] + self.appliance_names
                house_data = house_data[cols]

                # Clip low readings & apply cutoff
                house_data = house_data.clip(lower=0, upper=[self.cutoff[0]]*len(house_data.columns), axis=1)
                house_data[house_data < 5] = 0
                house_data = house_data[house_data['aggregate'] > 0]
            ...
            # Return aggregate as x and appliances as y
            x = entire_data['aggregate'].values
            y = entire_data[self.appliance_names].values
            return x, y
...
class UK_DALE_Dataset(AbstractDataset):
    ...
    def _if_data_exists(self):
        folder = Path("Enter the path of UK-DALE dataset.").joinpath(self.code())
        first_file = folder.joinpath('Enter the path of first file of the UK-DALE dataset.')
            ...
            for house_id in self.house_indicies:
                ...
                for appliance in self.appliance_names:
                    if app_index_dict[appliance][0] == -1:
                        ...
                        #    method='ffill', limit=30)
                        # Replace the fillna method with forward fill (ffill)
                        temp_data = temp_data.resample(self.sampling).mean().ffill()
    ...
    def compute_status(self, data):
        ...
        for i in range(data.shape[1]):
            ...
            if len(on_events) > 0:
                ...
                on_duration = off_events - on_events
                mask = on_duration >= self.min_on[i]
                on_events = on_events[mask]
                off_events = off_events[mask]

            temp_status = np.zeros_like(data[:, i])
            for on, off in zip(on_events, off_events):
                temp_status[on:off] = 1
            status[:, i] = temp_status
        return status
    ...
    def get_datasets(self):
        val_end = int(self.val_size * len(self.x))
        val = NILMDataset(
            self.x[:val_end], self.y[:val_end], self.status[:val_end],
            self.window_size, self.window_size
        )
        train = NILMDataset(
            self.x[val_end:], self.y[val_end:], self.status[val_end:],
