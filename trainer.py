...
class Trainer(metaclass=ABCMeta):
    ...
    def train_one_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(tqdm_dataloader):
            ...
            tqdm_dataloader.set_description('Epoch {}, loss {:.6f}'.format(epoch, average_loss))
    ...
    def train_bert_one_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(tqdm_dataloader):
            ...
            average_loss = np.mean(np.array(loss_values))
            tqdm_dataloader.set_description('Epoch {}, loss {:.7f}'.format(epoch, average_loss))
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                if logits is not None:
                  ...
                  # Handle the case when logits is None
                  raise ValueError("Logits is None, check your model output.")
                ...
                tqdm_dataloader.set_description('Validation, rel_err {:.7f}, acc {:.7f}, f1 {:.7f}'.format(
    ...
    def _load_best_model(self):
        try:
            self.model.load_state_dict(torch.load(os.path.join(
                    self.export_root, 'best_acc_model.pth'), map_location='cpu'))
